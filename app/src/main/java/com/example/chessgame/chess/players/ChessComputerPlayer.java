...
public class ChessComputerPlayer extends GameComputerPlayer {
     ...
     */
    public ChessComputerPlayer(String name, ChessState state) {
    ...
    protected void receiveInfo(GameInfo info) {
        ...
        if (state.getPiece(xVal, yVal).getPieceColor() == Piece.ColorType.WHITE && state.getWhoseMove() == 0) {
            game.sendAction(new ChessSelectAction(this, yVal, xVal));
        } else if (state.getPiece(xVal, yVal).getPieceColor() == Piece.ColorType.BLACK && state.getWhoseMove() == 1) {
            game.sendAction(new ChessSelectAction(this, yVal, xVal));
        } else if (state.getPiece(xVal, yVal).getPieceColor() != Piece.ColorType.WHITE && state.getWhoseMove() == 0) {
